...
class Config:
    ...
    SYSTEM_PROMPT = (
        ...
        "Speak warmly in simple English, 2–3 sentences. "
        "Offer grounding or breathing guidance when helpful. "
        "Be brief, empathetic, and natural. No medical advice."
    )
...
class RealtimeClient:
    def __init__(self, sid, on_audio, on_text_delta, on_text_done, on_error):
        self.sid = sid
        self.ws = None
    ...
    async def connect(self):
        ...
        try:
            self.ws = await websockets.connect(
                Config.REALTIME_URL, extra_headers=headers, ping_interval=10, ping_timeout=5
            )
        ...
        try:
            async for raw in self.ws:
                ...
                if t == "response.audio.delta":
                    ...
                    await self.on_text_delta(e.get("delta") or "")
                elif t == "response.audio_transcript.done":
                    await self.on_text_done(e.get("transcript") or "")
                elif t == "response.done":
                    self.response_in_progress = False
                elif t == "error":
                    await self.on_error(e.get("error", {}).get("message", "Unknown error"))
        except Exception as ex:
            log.error(f"Recv error {self.sid}: {ex}")
    ...
    async def commit(self):
        ...
        if now - self.last_commit_time < self.commit_cooldown or self.response_in_progress:
            return
...
# ---------- FASTAPI APP ----------
Config.validate()
app = FastAPI(title="Mindful+ Voice", version="2.1.0")
        ...
        while True:
            ...
            if t == "start_session":
                ...
                if await client.connect():
                    ...
                    await safe_send(ws, {"type": "error", "message": "Failed to connect"})

            elif t == "audio_data" and (c := SESSIONS[sid].get("client")):
                await c.send_audio(msg.get("data", ""))

            elif t == "stop_session" and (c := SESSIONS[sid].get("client")):
                await c.commit()
                await c.close()
                break
...
async def send_audio(sid, audio): 
    if s := SESSIONS.get(sid): await safe_send(s["ws"], {"type": "audio_response", "data": audio.hex()})
async def send_text_delta(sid, delta): 
    if s := SESSIONS.get(sid): await safe_send(s["ws"], {"type": "transcript_delta", "delta": delta})
async def send_text_done(sid, txt): 
    if s := SESSIONS.get(sid): await safe_send(s["ws"], {"type": "transcript", "text": txt})
async def send_err(sid, err): 
    if s := SESSIONS.get(sid): await safe_send(s["ws"], {"type": "error", "message": err})
async def cleanup(sid):
    if s := SESSIONS.pop(sid, None):
        if c := s.get("client"): await c.close()
